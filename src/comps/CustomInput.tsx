...
export default function CustomInput({
  ...
  return (
    <div className={`relative z-1 ${className}`}>
      ...
      {isOptionsOpen && (
        <div
          className={`bg-beach rounded-[12px] absolute w-full lg:p-[12px] p-[4px] overflow-auto`}
        >
          {options.map((option: GettingData, index: number) => (
            <div
              ...
              className={`${option.title === value && "z-100 bg-[#80011F0D] text-red-accent font-bold"} mb-[4px] lg:mb-[8px] unselectable h-[34px] lg:h-[45px] p-[12px] text-left flex items-center lg:text-[16px] text-[14px] font-normal font-roboto rounded-[12px] `}
            >
              {option.title}
            </div>
          ))}
